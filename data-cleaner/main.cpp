...
int main (int argc, char** argv) {
	...
	inFileName  = "./clean-data/" + (string)argv[1];
	outFileName = "./processed-data/" + (string)argv[1];
	...
	while (in >> aside >> netid >> nodes >> power >> alpha >> betaa >> links >> fsets >> fract >> multi >> zLP >> zIP >> ratio >> enumt >> linpt >> intpt >> sflag) {
		if (!sflag) {
			...
			out << aside << "\t" << netid << "\t" << nodes << "\t" << power << "\t" << alpha << "\t" << betaa << "\t" << links << "\t" << fsets << "\t" << fract << "\t" << multi << "\t" << zLP << "\t" << zIP << "\t" << ratio << "\t" << enumt << "\t" << linpt << "\t" << intpt << "\t" << sflag << "\t" << endl;
		}		
	}
